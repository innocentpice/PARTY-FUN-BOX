...
export const spotifyApi = SpotifyApi.withImplicitGrant(
  ...
  'https://45scl2-4200.csb.app/v3/player',
  Scopes.all
);
...
export function SpotifyPlayer() {
  ...
  React.useEffect(() => {
    ...
    window.onSpotifyWebPlaybackSDKReady = async () => {
      ...
      const access_token =
        ...
        (await spotifyApi.getAccessToken())?.access_token;
      ...
      player.addListener('authentication_error', () => {
        ...
        window.location.replace('/v3/player');
      });
  ...
  React.useEffect(() => {
    ...
    if (!track) {
      spotifyApi.recommendations
        ...
        .then((res) => {
          ...
          tracks.map((track) => {
            realmCollections?.playlist?.updateOne(
              { source: track.source, id: track.id },
              { $set: track },
              { upsert: true }
            );
          });
    ...
    if (
      spotifyPlayer.playerState?.track_window?.current_track?.uri !== track.uri
    )
      spotifyApi.player.startResumePlayback(
        spotifyPlayer.device_id,
        undefined,
        [track.uri]
      );
